...
export function PanelUnavailable({
  title = 'API de patrocinios no conectada',
  onRetry,
}: {
  title?: string;
  onRetry?: () => void;
  ...
  return (
    <EmptyState
      ...
      illustration="inbox"
      title={title}
      description="Este panel consulta /sponsorships. Sin respuesta no se inventan contratos, assets ni ROI."
      hints={[
...
export function PanelError({
  ...
  return (
    <EmptyState
      ...
      description={sponsorshipsErrorMessage(error)}
      action={
        onRetry ? (
          <Button variant="secondary" size="sm" onClick={onRetry}>
            Reintentar
