...
#   docker build -f docker/web.Dockerfile --ignorefile docker/.dockerignore \
#     --build-arg NEXT_PUBLIC_API_URL=https://api.example.com/api/v1 \
#     -t boletera-web .
#
# BLOCKER: apps/web/next.config.ts has no `output: 'standalone'`.
...
FROM base AS pruner
COPY . .
RUN pnpm dlx turbo@2.9.14 prune @boletera/web --docker
...
# Root postinstall runs db:generate; web prune has no @boletera/database.
...
RUN pnpm turbo run build --filter=@boletera/web...

# deploy omits gitignored .next; overlay after deploy
RUN pnpm --filter=@boletera/web deploy --legacy --prod /deploy \
  && cp -a apps/web/.next /deploy/.next \
  && if [ -d apps/web/public ]; then cp -a apps/web/public /deploy/public; fi \
  && cp apps/web/next.config.ts /deploy/next.config.ts \
...
# Not standalone — `next start` requires production node_modules + .next
